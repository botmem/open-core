...
export async function syncContacts(
  ...
  const total = totalRes.data.totalPeople || 0;
  ctx.logger.info(`Total contacts: ${total}`);
  ...
  do {
    ...
    const res = await people.people.connections.list({
      ...
      pageSize: PAGE_SIZE,
      pageToken,
      personFields: 'names,emailAddresses,phoneNumbers,organizations,addresses,birthdays,biographies,urls,relations,events,occupations,nicknames',
    });
    ...
    for (const person of connections) {
      ...
      const orgs = (person.organizations || []).map((o) => [o.name, o.title].filter(Boolean).join(' — ')).filter(Boolean);
      ...
      if (orgs.length) textParts.push(`Organization: ${orgs.join('; ')}`);
      ...
      emit({
        sourceType: 'message', // Using 'message' as closest available type for contacts
