...
export async function syncContacts(
  ...
  try {
    ...
    ctx.logger.info(`Total contacts: ${total}, first page connections: ${totalRes.data.connections?.length || 0}`);
  } catch (err: any) {
    ctx.logger.warn(`Failed to get contacts count: ${err.message}`);
  }
  ...
  do {
    ...
    try {
      res = await people.people.connections.list({
        ...
        pageSize: PAGE_SIZE,
        pageToken,
        personFields: 'names,emailAddresses,phoneNumbers,organizations,addresses,birthdays,biographies,urls,relations,events,occupations,nicknames,photos,sipAddresses,imClients,userDefined,memberships,externalIds,interests,genders',
      });
    } catch (err: any) {
      ctx.logger.error(`Contacts list failed: ${err.message}`);
      break;
    }

    const connections = res.data.connections || [];
    ctx.logger.info(`Contacts page: ${connections.length} connections, nextPage: ${res.data.nextPageToken ? 'yes' : 'no'}`);
    ...
    for (const person of connections) {
      ...
      const phones = (person.phoneNumbers || []).map((p) => `${p.value} (${p.type || 'other'})`).filter(Boolean);
      const orgs = (person.organizations || []).map((o) => ({
        name: o.name || undefined,
        title: o.title || undefined,
        department: o.department || undefined,
      })).filter((o) => o.name || o.title);
      const orgDisplay = orgs.map((o) => [o.name, o.title, o.department].filter(Boolean).join(' — ')).filter(Boolean);
      const addresses = (person.addresses || []).map((a) => a.formattedValue).filter(Boolean) as string[];
      const birthday = person.birthdays?.[0]?.date;
      const birthdayStr = birthday ? `${birthday.year || '????'}-${String(birthday.month || 1).padStart(2, '0')}-${String(birthday.day || 1).padStart(2, '0')}` : undefined;
      const bio = person.biographies?.[0]?.value;
      const urls = (person.urls || []).map((u) => u.value).filter(Boolean) as string[];
      const relations = (person.relations || []).map((r) => `${r.person} (${r.type || 'other'})`).filter(Boolean);
      const nicknames = (person.nicknames || []).map((n) => n.value).filter(Boolean) as string[];
      const photoUrl = person.photos?.[0]?.url || undefined;
      const sipAddresses = (person.sipAddresses || []).map((s) => s.value).filter(Boolean) as string[];
      const imClients = (person.imClients || []).map((im) => `${im.username} (${im.protocol || im.type || 'other'})`).filter(Boolean);
      const occupations = (person.occupations || []).map((o) => o.value).filter(Boolean) as string[];
      const interests = (person.interests || []).map((i) => i.value).filter(Boolean) as string[];
      const gender = person.genders?.[0]?.value || undefined;
      const externalIds = (person.externalIds || []).map((e) => `${e.value} (${e.type || 'other'})`).filter(Boolean);
      const userDefined = (person.userDefined || []).map((u) => `${u.key}: ${u.value}`).filter(Boolean);
      const events = (person.events || []).map((e) => {
        const d = e.date;
        const dateStr = d ? `${d.year || '????'}-${String(d.month || 1).padStart(2, '0')}-${String(d.day || 1).padStart(2, '0')}` : '';
