...
function createMockSocket() {
  ...
  socket.removeListener = vi.fn((event: string, fn: any) => {
    origRemove(event, fn);
    return socket;
  });
  return socket;
...
describe('ImsgClient', () => {
  ...
  describe('disconnect', () => {
    it('destroys socket and rejects pending requests', async () => {
      const client = await makeAndConnect();
      mockSocket.write.mockImplementation((_d: string, _e: string, cb: Function) => cb());
  ...
  describe('chatsList', () => {
    it('sends JSON-RPC request and returns chats', async () => {
      const client = await makeAndConnect();
      mockSocket.write.mockImplementation((data: string, _e: string, cb: Function) => {
        cb();
        const req = JSON.parse(data.trim());
        mockSocket.emit('data', Buffer.from(
          JSON.stringify({ jsonrpc: '2.0', id: req.id, result: { chats: [{ id: 1, name: 'Chat 1', identifier: '+1234', service: 'iMessage', last_message_at: '' }] } }) + '\n'
        ));
      });
  ...
  describe('messagesHistory', () => {
      ...
      mockSocket.write.mockImplementation((data: string, _e: string, cb: Function) => {
        ...
        mockSocket.emit('data', Buffer.from(
          JSON.stringify({
            ...
            result: { messages: [{ id: 1, chat_id: 5, guid: 'g1', sender: '+1234', is_from_me: false, text: 'Hello', created_at: '2025-01-01T12:00:00Z', attachments: [], reactions: [], chat_identifier: '+1234', chat_name: 'Chat', participants: ['+1234'], is_group: false }] },
          }) + '\n'
        ));
      });

      const msgs = await client.messagesHistory(5, { limit: 100, start: '2025-01-01', end: '2025-12-31', attachments: true });
    ...
    it('sends minimal params when no options', async () => {
      ...
      mockSocket.write.mockImplementation((data: string, _e: string, cb: Function) => {
        cb();
        const req = JSON.parse(data.trim());
        mockSocket.emit('data', Buffer.from(JSON.stringify({ jsonrpc: '2.0', id: req.id, result: { messages: [] } }) + '\n'));
      });
  ...
  describe('error handling', () => {
    it('rejects with RPC error', async () => {
      ...
      mockSocket.write.mockImplementation((data: string, _e: string, cb: Function) => {
        ...
        const req = JSON.parse(data.trim());
        mockSocket.emit('data', Buffer.from(
          JSON.stringify({ jsonrpc: '2.0', id: req.id, error: { code: -32600, message: 'Invalid request', data: { detail: 'bad' } } }) + '\n'
        ));
      });
    ...
    it('rejects on write error', async () => {
      const client = await makeAndConnect();
      mockSocket.write.mockImplementation((_d: string, _e: string, cb: Function) => cb(new Error('Write failed')));
  ...
  describe('buffer processing', () => {
    it('handles partial data across multiple chunks', async () => {
      ...
      const response = JSON.stringify({
        ...
        result: { chats: [{ id: 1, name: 'Test', identifier: 'x', service: 'iMessage', last_message_at: '' }] },
      });
    ...
    it('handles malformed JSON lines gracefully', async () => {
      ...
      const chatsPromise = client.chatsList();
      const validResponse = JSON.stringify({ jsonrpc: '2.0', id: requestId, result: { chats: [] } });
    ...
    it('ignores responses for unknown request ids', async () => {
      ...
      const rightResponse = JSON.stringify({ jsonrpc: '2.0', id: requestId, result: { chats: [] } });
