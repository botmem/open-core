...
describe('WhatsAppConnector', () => {
  ...
  describe('embed', () => {
    it('extracts sender from senderPhone with name', () => {
      const event = {
        sourceType: 'message' as const, sourceId: 'wa1', timestamp: '2026-01-01T00:00:00Z',
        content: { text: 'Hi', participants: [], metadata: { senderPhone: '1234567890', senderName: 'Alice', fromMe: false } },
      };
      const result = connector.embed(event, 'Hi', {} as any);
      expect(result.entities).toContainEqual({ type: 'person', id: 'phone:1234567890|name:Alice', role: 'sender' });
    });

    it('extracts group entity when isGroup with chatId', () => {
      const event = {
        sourceType: 'message' as const, sourceId: 'wa1', timestamp: '2026-01-01T00:00:00Z',
        content: { text: 'Hi', participants: [], metadata: { isGroup: true, chatId: '120363@g.us', chatName: 'Family' } },
      };
      const result = connector.embed(event, 'Hi', {} as any);
    ...
    it('extracts DM recipient when not group', () => {
      const event = {
        sourceType: 'message' as const, sourceId: 'wa1', timestamp: '2026-01-01T00:00:00Z',
        content: { text: 'Hi', participants: ['1234567890@s.whatsapp.net'], metadata: { senderPhone: '1234567890', selfPhone: '9876543210', fromMe: true, isGroup: false } },
      };
      const result = connector.embed(event, 'Hi', {} as any);
    ...
    it('extracts recipient in DM when not fromMe', () => {
      const event = {
        sourceType: 'message' as const, sourceId: 'wa1', timestamp: '2026-01-01T00:00:00Z',
        content: { text: 'Hi', participants: [], metadata: { senderPhone: '1234567890', selfPhone: '9876543210', fromMe: false, isGroup: false } },
      };
      const result = connector.embed(event, 'Hi', {} as any);
    ...
    it('extracts mentions', () => {
      const event = {
        sourceType: 'message' as const, sourceId: 'wa1', timestamp: '2026-01-01T00:00:00Z',
        content: { text: '@Bob', participants: [], metadata: { mentions: [{ phone: '5551234', name: 'Bob' }] } },
      };
    ...
    it('extracts shared contacts from vCards', () => {
      const event = {
        sourceType: 'message' as const, sourceId: 'wa1', timestamp: '2026-01-01T00:00:00Z',
        content: { text: 'Contact card', participants: [], metadata: { sharedContacts: [{ name: 'Carol', phones: ['5559876'] }] } },
      };
      const result = connector.embed(event, 'Contact card', {} as any);
      expect(result.entities).toContainEqual({ type: 'person', id: 'name:Carol|phone:5559876', role: 'mentioned' });
    });
    ...
    it('skips sender name that equals phone or "me"', () => {
      const event = {
        sourceType: 'message' as const, sourceId: 'wa1', timestamp: '2026-01-01T00:00:00Z',
        content: { text: 'Hi', participants: [], metadata: { senderPhone: '1234567890', senderName: 'me' } },
      };
      const result = connector.embed(event, 'Hi', {} as any);
