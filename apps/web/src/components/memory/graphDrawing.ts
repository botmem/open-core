...
export { CONTACT_COLOR, SELF_COLOR, GROUP_COLOR, FILE_COLOR, DEVICE_COLOR, HIGHLIGHT_COLOR, DIM_OPACITY };
...
export function renderNode(node: any, ctx: CanvasRenderingContext2D, globalScale: number, rc: NodeRenderCtx) {
  const x = node.x || 0;
  const y = node.y || 0;
  ...
  ctx.globalAlpha = shouldDim ? DIM_OPACITY : isSearchActive && isDirectMatch && !isTopResult ? 0.4 + rankScore * 0.6 : 1;

  if (isConnector) {
    const color = CONNECTOR_COLORS[node.source] || '#999';
    const w = 28; const h = 20; const r = 5;
    drawRoundedRect(ctx, x + 2, y + 2, w, h, r);
    ctx.fillStyle = 'rgba(255,255,255,0.15)'; ctx.fill();
    drawRoundedRect(ctx, x, y, w, h, r);
    ctx.fillStyle = color; ctx.fill();
    if (isDirectMatch) { ctx.strokeStyle = HIGHLIGHT_COLOR; ctx.lineWidth = 3; }
    else { ctx.strokeStyle = '#E0E0E0'; ctx.lineWidth = 2; }
    ctx.stroke();
    ...
    ctx.fillStyle = FILE_COLOR; ctx.fill();
    if (isDirectMatch) { ctx.strokeStyle = HIGHLIGHT_COLOR; ctx.lineWidth = 3; }
    else { ctx.strokeStyle = '#E0E0E0'; ctx.lineWidth = 1.5; }
    ctx.stroke();
    ...
    ctx.fillStyle = GROUP_COLOR; ctx.fill();
    if (isDirectMatch) { ctx.strokeStyle = HIGHLIGHT_COLOR; ctx.lineWidth = 3; }
    else { ctx.strokeStyle = '#E0E0E0'; ctx.lineWidth = 2; }
    ctx.stroke();
    ctx.fillStyle = '#1A1A2E';
    ctx.beginPath(); ctx.arc(x - 2.5, y - 1, 2, 0, 2 * Math.PI); ctx.fill();
    ctx.beginPath(); ctx.arc(x + 2.5, y - 1, 2, 0, 2 * Math.PI); ctx.fill();
    ctx.beginPath(); ctx.arc(x, y + 4, 5, Math.PI, 0); ctx.fill();
    if (globalScale > 1.0 || isDirectMatch) {
      ctx.font = `bold ${10 / globalScale}px IBM Plex Mono`;
    ...
    const w = 20; const h = 14; const r = 4;
    drawRoundedRect(ctx, x + 1.5, y + 1.5, w, h, r);
    ctx.fillStyle = 'rgba(255,255,255,0.12)'; ctx.fill();
    drawRoundedRect(ctx, x, y, w, h, r);
    ctx.fillStyle = DEVICE_COLOR; ctx.fill();
    if (isDirectMatch) { ctx.strokeStyle = HIGHLIGHT_COLOR; ctx.lineWidth = 3; }
    else { ctx.strokeStyle = '#E0E0E0'; ctx.lineWidth = 1.5; }
    ctx.stroke();
    ctx.fillStyle = '#1A1A2E';
    ...
    ctx.beginPath(); ctx.arc(x + 1.5, y + 1.5, radius, 0, 2 * Math.PI);
    ctx.fillStyle = 'rgba(255,255,255,0.12)'; ctx.fill();
    ctx.beginPath(); ctx.arc(x, y, radius, 0, 2 * Math.PI);
    ctx.fillStyle = contactColor; ctx.fill();
    if (isDirectMatch) { ctx.strokeStyle = HIGHLIGHT_COLOR; ctx.lineWidth = 3; }
    else if (isSelf) { ctx.strokeStyle = '#FFF'; ctx.lineWidth = 3; }
    else { ctx.strokeStyle = '#E0E0E0'; ctx.lineWidth = 2; }
    ctx.stroke();
    if (isSelf) {
    ...
    const color = CONNECTOR_COLORS[node.source] || '#999';
    if (isTopResult) { ctx.shadowColor = HIGHLIGHT_COLOR; ctx.shadowBlur = 8 + rankScore * 12; }
    ctx.fillStyle = 'rgba(255,255,255,0.12)';
    ctx.fillRect(x - size / 2 + 2, y - size / 2 + 2, size, size);
    ctx.fillStyle = color;
    ctx.fillRect(x - size / 2, y - size / 2, size, size);
    if (isTopResult) { ctx.shadowColor = 'transparent'; ctx.shadowBlur = 0; }
    if (isDirectMatch) { ctx.strokeStyle = HIGHLIGHT_COLOR; ctx.lineWidth = isTopResult ? 4 : 3; }
    else { ctx.strokeStyle = '#E0E0E0'; ctx.lineWidth = 1.5; }
...
export function renderNodePointerArea(node: any, color: string, ctx: CanvasRenderingContext2D) {
  ...
  const y = node.y || 0;
  ctx.fillStyle = color;
  if (node.nodeType === 'connector') { drawRoundedRect(ctx, x, y, 32, 24, 6); ctx.fill(); }
  else if (node.nodeType === 'file') { drawDiamond(ctx, x, y, 9); ctx.fill(); }
  else if (node.nodeType === 'group') { drawHexagon(ctx, x, y, 12); ctx.fill(); }
  else if (node.nodeType === 'device') { drawRoundedRect(ctx, x, y, 24, 18, 5); ctx.fill(); }
  else if (node.nodeType === 'contact') { ctx.beginPath(); ctx.arc(x, y, 10, 0, 2 * Math.PI); ctx.fill(); }
  else { const size = 6 + (node.importance || 0.5) * 12; ctx.fillRect(x - size / 2, y - size / 2, size, size); }
}
