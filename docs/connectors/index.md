...
Connectors are registered in the `ConnectorRegistry` at startup. Each connector provides a manifest describing its ID, name, auth type, and configuration schema.

### 2. Authentication

Depending on the auth type:
- **OAuth 2.0** -- User is redirected to the service's consent screen, then back to Botmem with an authorization code
...
|---|---|---|
| `email` | Email messages with headers, body, and recipients | Gmail |
| `message` | Chat messages with sender and channel | Slack, WhatsApp, iMessage |
...
See [Building a Connector](/connectors/building-a-connector) for a complete guide to creating a custom connector.
